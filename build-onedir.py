...
def copy_required_files():
    """将 playwright-browsers、data 和 config.json 复制到 dist/BankSync/ 目录"""
    project_root = os.path.abspath(os.path.dirname(__file__))
    dist_dir = os.path.join(project_root, "dist", "BankSync")
    required_items = [
        "playwright-browsers",
        "data",
        "config.json"
...
def run_pyinstaller():
    ...
    try:
        ...
        print("打包完成！可执行文件位于: dist/BankSync/BankSync.exe")
        print("复制必要的文件和文件夹到 dist/BankSync/ 目录...")
        copy_required_files()
        print("所有文件已复制到 dist/BankSync/ 目录！")
