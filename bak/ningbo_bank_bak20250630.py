...
from utils import log, read_bank_config, find_and_click_image, handle_overwrite_dialog, handle_save_dialog
import os
import time
        ...
        for index, (xiangmu, account) in enumerate(projects_accounts):
            ...
            try:
                ...
                try:
                    ...
                    time.sleep(5)  # 等待图像加载
                    duizhangdan_button_path = get_resource_path("ningbo_duizhangdandayin.bmp")
                    ...
                    if find_and_click_image(duizhangdan_button_path, max_attempts=15):
                        log_local("成功点击‘对账单打印’按钮")
                    else:
                        log_local("未找到‘对账单打印’按钮")
                        pyautogui.screenshot(os.path.join(base_path, f"error_duizhangdan_button_{xiangmu}.png"))
                        continue
