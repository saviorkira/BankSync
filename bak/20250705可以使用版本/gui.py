...
from flet import (
    ...
    ElevatedButton, Dropdown, DataTable, DataColumn, DataRow, DataCell, TextField
)
...
def main(page: Page):
    ...
    data_table = ft.DataTable(
        columns=[
            ...
            ft.DataColumn(ft.Text("银行账号", weight=ft.FontWeight.BOLD)),
        ],
        rows=[],
        expand=True,
        border=ft.border.all(1, ft.Colors.GREY_300),  # ✅ 修复这里
        bgcolor=ft.Colors.WHITE,
    ...
    def run_export(e):
        ...
        def worker():
            try:
                project_root = os.path.dirname(sys.executable) if getattr(sys, 'frozen', False) else os.path.abspath(
                ...
                with sync_playwright() as playwright:
                    run_ningbo_bank(playwright, project_root, base_path, excel_data, kaishi, jieshu,
    ...
    page.add(
        ft.Container(
            content=ft.Column(
                [
                    ft.Text("银行流水与回单导出系统", size=24, weight=ft.FontWeight.BOLD, color=ft.Colors.BLUE_900),
                    ...
                    run_button,
                    ft.Text("导入数据:", size=16, weight=ft.FontWeight.BOLD, color=ft.Colors.GREY_800),
                    ft.Container(
                        content=data_table,
                        padding=10,
                        border_radius=8,
                    ...
                    ft.Text("运行日志:", size=16, weight=ft.FontWeight.BOLD, color=ft.Colors.GREY_800),
                    ft.Container(
                        content=log_area,
                        padding=10,
                        border_radius=8,
