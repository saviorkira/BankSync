...
def main(page: Page):
    ...
    bank_dropdown = ft.Dropdown(
        label="选择银行",
        options=[ft.dropdown.Option("ningbo_bank", "宁波银行")],
        value=None,
    ...
    def on_bank_select(e):
        if bank_dropdown.value:
            update_log(f"已选择银行: {bank_dropdown.options[0].text if bank_dropdown.value == 'ningbo_bank' else '未知'}")
        else:
            update_log("银行选择已清空")
    ...
    def run_export(e):
        ...
        if not bank_dropdown.value:
            ...
            return
        if bank_dropdown.value != "ningbo_bank":
        ...
        if os.path.exists(login_dir):
            for file in os.listdir(login_dir):
                if file.lower().endswith(('.png', '.jpg', '.jpeg')):
                    ...
                    bank_name = os.path.splitext(file)[0]  # 保持原始文件名，如 ningbo_bank
                    ...
                    icon = ft.Image(
                        ...
                        tooltip=f"登录 {bank_name.replace('_', ' ')}",
                    )
